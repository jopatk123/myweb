 ...
 * 复制预选图标到 uploads 目录。
 *
 * - 仅在 publicIconsDir / distIconsDir / presetIconsDir 中查找，避免从 uploadsDir 复制
...
export async function copyPresetAppIcon({
  ...
  if (!safeFilename) {
    throw new ValidationError('预选图标文件名为空');
  }

  const sourceCandidates = [
    path.join(publicIconsDir, safeFilename),
    ...(distIconsDir ? [path.join(distIconsDir, safeFilename)] : []),
  ...
  if (!sourcePath) {
    throw new ValidationError(`预选图标不存在: ${safeFilename}`);
  }
