...
export function joinLeaveRoomFactory(Service){
  ...
  Service.prototype.joinRoom = function(sessionId, playerName, roomCode) {
    try {
      ...
      if (room.status === 'playing' && !isSnakeSharedMode) throw new Error('游戏正在进行中，无法加入');
      const existingPlayer = this.PlayerModel.findByRoomAndSession(room.id, sessionId);
      if (existingPlayer) {
        this.PlayerModel.update(existingPlayer.id, { is_online: true, last_active: new Date().toISOString() });
        const players = this.PlayerModel.findOnlineByRoomId(room.id);
        this.broadcastToRoom(room.id, 'player_reconnected', { player: existingPlayer, room_id: room.id });
      ...
      const player = this.PlayerModel.create({ room_id: room.id, session_id: sessionId, player_name: playerName, player_color: playerColor, is_ready: false });
      this.RoomModel.update(room.id, { current_players: currentPlayers + 1 });
      const players = this.PlayerModel.findOnlineByRoomId(room.id);
      this.broadcastToRoom(room.id, 'player_joined', { player, room_id: room.id, players, room });
      console.log(`玩家 ${playerName} 加入 ${this.getGameType()} 房间: ${roomCode} (状态: ${room.status})`);
      return { room, player, players };
    } catch (error) { console.error('加入房间失败:', error); throw error; }
  };
  ...
  Service.prototype.transferHostRole = function(roomId){
    try { const players = this.PlayerModel.findOnlineByRoomId(roomId); if (players.length > 0) { const newHost = players[0]; this.RoomModel.update(roomId, { created_by: newHost.session_id }); this.broadcastToRoom(roomId, 'host_changed', { new_host: newHost.session_id, room_id: roomId }); console.log(`房间 ${roomId} 房主已转移给 ${newHost.player_name}`); } } catch (error) { console.error('转移房主权限失败:', error); }
  };
}
