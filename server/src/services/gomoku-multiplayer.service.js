export class GomokuMultiplayerService {
  ...
  joinRoom(sessionId, playerName, roomCode) {
    ...
    if (room.players.find(p => p.session_id === clientSessionId))
  ...
  toggleReady(sessionId, roomCode) {
    ...
    // 向房间内所有玩家广播状态变化
    this._broadcastRoom(room, 'player_ready_changed', {
      player: p,
  ...
  placePiece(sessionId, roomCode, row, col) {
    ...
    // 获取客户端sessionId用于匹配
    const clientSessionId = this.ws.serverToClient.get(sessionId) || sessionId;
    const player = room.players.find(p => p.session_id === clientSessionId);
    if (!player) return;
    if (gs.winner) return;
    if (gs.currentPlayer !== player.seat) return; // not turn
    if (row < 0 || row >= 15 || col < 0 || col >= 15) return;
    if (gs.board[row][col] !== 0) return;
    gs.board[row][col] = player.seat;
    gs.lastMove = { row, col, player: player.seat };
    // check win
    if (this._checkWin(gs.board, row, col, player.seat)) {
      gs.winner = player.seat;
      room.status = 'finished';
      this._broadcastRoom(room, 'game_ended', { game_state: gs });
      return;
    }
  ...
  async handlePlayerDisconnect(sessionId) {
    try {
      const clientSessionId =
        this.ws.serverToClient.get(sessionId) || sessionId;
      for (const [roomCode, room] of Array.from(this.rooms.entries())) {
        const idx = room.players.findIndex(
          p => p.session_id === clientSessionId || p.session_id === sessionId
        );
        if (idx >= 0) {
          const [player] = room.players.splice(idx, 1);
          console.debug(
  ...
  getRoomInfo(roomCode) {
    const room = this.rooms.get(roomCode);
    if (!room) return null;
    return { room, players: room.players, game_state: room.game_state };
  }
  ...
  _genCode() {
    ...
    if (this.rooms.has(code)) return this._genCode();
    return code;
  }
  _createEmptyGameState() {
    return {
  ...
  _checkWin(board, row, col, player) {
    const dirs = [
      [1, 0],
      [0, 1],
      [1, 1],
    ...
    const inB = (r, c) => r >= 0 && r < 15 && c >= 0 && c < 15;
    for (const [dr, dc] of dirs) {
      let cnt = 1;
      let r = row + dr,
        c = col + dc;
      while (inB(r, c) && board[r][c] === player) {
        cnt++;
        r += dr;
        c += dc;
      }
