...
import {
  createInitialSnake,
  updateSharedGameTick,
} from './shared-mode.logic.js';
...
export class SnakeGameService extends RoomManagerService {
  ...
  createSnakeRoom(sessionId, playerName, mode, gameSettings = {}) {
    ...
    const roomData = {
      mode,
      max_players: mode === 'shared' ? 999 : mode === 'competitive' ? 8 : 2,
      status: 'waiting',
    };
    const gameConfig = {
  ...
  initGameState(roomId, mode, config = {}) {
    ...
    if (mode === 'shared') {
      gameState.sharedSnake = createInitialSnake(this.SNAKE_CONFIG.BOARD_SIZE);
  ...
  // ---- 通用工具 ----
  generateFood(exclude = []) {
    return this.generateRandomPosition(this.SNAKE_CONFIG.BOARD_SIZE, exclude);
  }
  isValidDirectionChange(cur, next) {
  ...
  startSharedLoop(roomId) {
    ...
    const loop = setInterval(
      () => updateSharedGameTick(this, roomId),
      this.SNAKE_CONFIG.GAME_SPEED
    );
    this.gameTimers.set(roomId, loop);
  }
  startCompetitiveLoop(roomId) {
    const tick = () => {
      ...
      updateCompetitiveGameTick(this, roomId);
      const latest = this.getGameState(roomId);
      if (latest && latest.status === 'playing') {
        this.gameTimers.set(
          roomId,
          setTimeout(tick, this.SNAKE_CONFIG.GAME_SPEED)
        );
      } else {
        this.gameTimers.delete(roomId);
      }
  ...
  autoCleanupStaleRooms() {
    ...
    rooms.forEach(r => {
      ...
      const parseDbTime = ts => {
        if (!ts) return now;
        let s = ts.trim();
        if (/T/.test(s) === false) s = s.replace(' ', 'T');
        if (!/Z$/i.test(s)) s += 'Z';
      ...
      if (
        ...
        console.warn(
          '[SnakeCleanup][TZ?] 发现可能的时区偏差导致的巨大 idleMs',
          {
            ...
            idleMs,
            updated_at: r.updated_at,
            parsedUpdatedAt: new Date(updatedAt).toISOString(),
