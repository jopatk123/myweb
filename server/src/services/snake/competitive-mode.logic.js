...
export function updateCompetitiveGameTick(service, roomId){
  const gameState = service.getGameState(roomId); if(!gameState) return; const size = gameState.config?.BOARD_SIZE || service.SNAKE_CONFIG.BOARD_SIZE;
  try { service.RoomModel.update(roomId, { /* tick touch */ }); } catch (e) { console.warn('competitive tick touch failed', e); }
  if(gameState.loser){ // 已经确定输家，等待 lifecycle endGame 调用（防抖）
    return;
  }
  ...
  if(loserDetected){
    ...
    // 赢家列表 = 其他所有玩家
    gameState.winners = Object.values(gameState.snakes).filter(s=>s!==loserDetected).map(s=>s.player);
    // 排行需要分数，服务端不再推迟，立即结束
