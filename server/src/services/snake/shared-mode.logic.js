...
export function createInitialSnake(boardSize){
  ...
  return {
    body:[{x:centerX,y:centerY},{x:centerX,y:centerY+1},{x:centerX,y:centerY+2}],
    direction:'right',
    nextDirection:'right',
    score:0,
    length:3,
...
export function updateSharedGameTick(service, roomId){
  ...
  if(snake.isWaitingForFirstVote){
    ...
    if(hasVotes || waitTime>5000 || singlePlayer){
      ...
      service.broadcastToRoom(roomId,'game_update',{room_id:roomId,game_state:gameState,shared_snake:snake,food:gameState.food,waiting_for_vote:true});
      return;
    }
  }
  ...
  if(service.checkSelfCollision(head,snake.body)){ service.endGame(roomId,'self_collision'); return; }
  const ate = head.x===gameState.food.x && head.y===gameState.food.y;
  snake.body.unshift(head);
  if(ate){ snake.score+=10; snake.length+=1; gameState.food = service.generateFood(snake.body);} else { snake.body.pop(); }
  service.broadcastToRoom(roomId,'game_update',{room_id:roomId,game_state:gameState,shared_snake:snake,food:gameState.food});
}
