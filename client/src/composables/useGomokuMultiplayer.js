...
export function useGomokuMultiplayer() {
  ...
  const bothReady = computed(
    () => players.value.filter(p => p.is_ready).length === 2
  );
  ...
  function registerHandlers() {
    ...
    const handlers = createGomokuHandlers({
      events,
      refs: {
        currentRoom,
        currentPlayer,
  ...
  function unregisterHandlers() {
    ...
    Object.entries(handlers).forEach(([type, h]) => {
      try {
        ...
        console.warn('[GomokuMP] Failed to remove handler', type, err);
      }
    });
    ...
    try {
      ...
      if (code) {
        try {
          ...
          console.error('[GomokuMP] Failed during unmount leaveRoom call', err);
        }
      }
    } catch (err) {
      console.error('[GomokuMP] Error while cleaning up on unmount', err);
    }
    unregisterHandlers();
  });
  ...
  if (typeof window !== 'undefined') {
    const beforeUnloadHandler = () => {
      try {
        ...
        console.error('[GomokuMP] beforeunload leaveRoom failed', err);
      }
    };
