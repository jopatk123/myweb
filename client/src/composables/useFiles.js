...
export function useFiles() {
  ...
  const totalPages = computed(() => Math.ceil((total.value || 0) / (limit.value || 1)));
  ...
  async function fetchList() {
    try {
      const raw = await filesApi.list({ page: page.value, limit: limit.value, type: type.value || undefined, search: search.value || undefined });
  ...
  return {
    items, page, limit, total, totalPages, type, search,
    uploading, uploadProgress, error,
    fetchList, upload, remove, getDownloadUrl,
