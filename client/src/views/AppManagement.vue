  ...
  <div class="admin-layout">
    ...
    <aside class="module-sider">
      <div class="module-header-row">
        ...
        <div class="header-actions">
          <button class="btn btn-secondary btn-sm" @click="/* 打开创建分组 */ null">新建</button>
        </div>
      </div>
      <div class="group-list">
        <div class="group-item" :class="{ active: selectedGroupId === '' }" @click="selectGroup('')">全部应用</div>
        <div class="group-item" v-for="g in groups" :key="g.id" :class="{ active: selectedGroupId === g.id }" @click="selectGroup(g.id)">
          {{ g.name }}
        </div>
      </div>
    </aside>
    ...
    <main class="content-area">
      <div class="toolbar-row">
        <div class="left">
          <button class="btn btn-primary" disabled title="暂未开放，内部应用由我们手工实现">新增应用</button>
          <button class="btn btn-secondary" :disabled="selectedIds.length===0" @click="visibleBulk(true)">设为可见</button>
          <button class="btn btn-secondary" :disabled="selectedIds.length===0" @click="visibleBulk(false)">设为隐藏</button>
        </div>
        <div class="right">
        ...
        <table>
          ...
          <tbody>
            <tr v-for="app in filteredApps" :key="app.id">
              <td><input type="checkbox" :value="app.id" v-model="selectedIds" /></td>
              <td>
                <img v-if="app.icon_filename" :src="getAppIconUrl(app)" alt="icon" class="icon" />
              ...
              <td>
                <label class="switch">
                  <input type="checkbox" :checked="!!app.is_visible" @change="onToggleVisible(app, $event.target.checked)" />
                  <span class="slider"></span>
                </label>
              </td>
              <td>
                <button class="btn btn-sm btn-secondary" @click="/* 打开编辑 */ null">编辑</button>
                <button class="btn btn-sm btn-danger" @click="remove(app.id)">删除</button>
              </td>
            </tr>
          </tbody>
        </table>
...
const filteredApps = computed(() => {
  ...
  return apps.value.filter(a => {
    const matchKeyword = !k || (a.name || '').toLowerCase().includes(k) || (a.slug || '').toLowerCase().includes(k);
    const matchGroup = !selectedGroupId.value || a.group_id === selectedGroupId.value;
    return matchKeyword && matchGroup;
  });
});

const allSelected = computed(() => filteredApps.value.length > 0 && selectedIds.value.length === filteredApps.value.length);
...
async function nextPage() {
  const totalPages = Math.max(1, Math.ceil((total.value || 0) / (limit.value || 20)));
...
const totalPages = computed(() => Math.max(1, Math.ceil((total.value || 0) / (limit.value || 20))));
...
.group-item { padding: 8px 10px; cursor: pointer; border-radius: 6px; }
...
.icon.placeholder { display: inline-block; width: 32px; height: 32px; text-align: center; color: #888; }
