      ...
      <div class="modal-body">
        <form @submit.prevent="handleUploadAll">
          ...
          <div class="form-group">
            ...
            <FileDropzone class="dropzone" accept="image/*" :multiple="true" @files-selected="handleFiles">
              <span v-if="selectedFiles.length === 0">
                点击选择或拖拽多个图片文件到此处<br>
                <small>最小分辨率：800x600，超过7680x4320会自动压缩；请保证单文件 &lt;= 10MB</small>
              </span>
...
const overallProgress = computed(() => {
  ...
  const sum = selectedFiles.value.reduce((acc, it) => acc + (it.progress || 0), 0);
...
const handleUploadAll = async () => {
  ...
  try {
    ...
    for (let i = 0; i < selectedFiles.value.length; i++) {
      ...
      item.progress = 0;
      await uploadWallpaper(item.file, selectedGroupId.value || null, item.name || item.originalName || '', (p) => {
...
.modal-content { background: white; border-radius: 8px; width: 90%; max-width: 900px; max-height: 80vh; overflow-y: auto; }
...
.dropzone { display:block; border:2px dashed #d0d7de; background:linear-gradient(180deg,#fbfcfd,#ffffff); padding:18px; border-radius:8px; text-align:center; }
...
.error-message { background:#fee; color:#c33; padding:10px; border-radius:4px; margin-bottom:12px; }
</style>
