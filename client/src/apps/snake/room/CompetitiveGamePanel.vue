    ...
    <div class="canvas-container">
      ...
      <div v-if="showSummary" class="finish-overlay">
        <div class="finish-card">
          <button class="close-btn" @click="showSummary = false" title="关闭">
            ✕
          </button>
          <h3>🏁 对局结束</h3>
          <p v-if="loserName">
            失败者：<strong style="color: #ef4444">{{ loserName }}</strong>
          </p>
          <p v-else-if="winnerName">
            胜者：<strong>{{ winnerName }}</strong>
          </p>
          <p v-else>本局未产生失败者</p>
          <div class="meta-line" v-if="durationSec > 0">
            耗时 {{ durationSec }}s · 总分 {{ totalScore }} · 食物
            {{ totalFood }}
          </div>
          <div class="summary-table">
            ...
            <div
              v-for="s in snakeListSorted"
              :key="s.player?.session_id"
              class="summary-row"
              :class="{ dead: s.gameOver }"
            >
              <span class="player" :title="s.player?.player_name">{{
                s.player?.player_name || '玩家'
              }}</span>
              <span>{{ s.score }}</span>
...
<script setup>
  ...
  const boardPx = computed(() =>
    Math.max(200, Math.floor(cell.value * gridSize.value))
  );
  ...
  const snakeListSorted = computed(() =>
    [...snakeList.value].sort((a, b) => b.score - a.score)
  );
  const totalScore = computed(() =>
    snakeList.value.reduce((a, b) => a + (b.score || 0), 0)
  );
  ...
  const winnerName = computed(
    () =>
      props.gameState?.winner?.player_name ||
      props.gameState?.winner?.playerName ||
      null
  );
  const loserName = computed(
    () =>
      props.gameState?.loser?.player_name ||
      props.gameState?.loser?.playerName ||
      null
  );
  const durationSec = computed(() => {
    const st = props.gameState?.startTime;
    const et = props.gameState?.endTime;
    if (!st) return 0;
    return Math.round(((et || Date.now()) - st) / 1000);
  });

  const showSummary = ref(false);
  watch(isGameFinished, v => {
    if (v) showSummary.value = true;
  });
  watch(
    () => props.gameState?.status,
    v => {
      if (v === 'playing') showSummary.value = false;
    }
  );
