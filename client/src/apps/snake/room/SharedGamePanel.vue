<template>
  <div class="shared-game-panel">
    <div class="game-canvas-area">
      <h4>🐍 共享蛇游戏</h4>
      <div class="vote-timer">投票倒计时: {{ voteCountdown }}s</div>
      ...
      <div class="game-info">
        ...
        <div
          class="info-item"
          v-if="gameState?.sharedSnake?.isWaitingForFirstVote"
        >
    ...
    <div v-if="showSummary" class="summary-overlay">
      <div class="summary-card">
        ...
        <div class="stats-grid">
          ...
          <div class="stat-item">
            <span class="label">吃到食物</span
            ><span class="val">{{ finalFood }}</span>
          </div>
          <div class="stat-item">
            <span class="label">长度</span
            ><span class="val">{{ finalLength }}</span>
          </div>
          <div class="stat-item">
            <span class="label">耗时</span
            ><span class="val">{{ durationSec }}s</span>
          </div>
        </div>
        <div class="players-box" v-if="playerCount > 0">
          <div class="players-title">参与玩家 ({{ playerCount }})</div>
          <ul class="players-list">
            <li v-for="p in players" :key="p.session_id">
              {{ p.player_name || p.playerName || '玩家' }}
            </li>
          </ul>
        </div>
        <div class="actions">
...
<script setup>
  ...
  const finalLength = computed(
    () =>
      props.gameState?.sharedSnake?.body?.length ||
      props.gameState?.sharedSnake?.length ||
      0
  );
  const durationSec = computed(() => {
    const start = props.gameState?.startTime;
    const end = props.gameState?.endTime;
    if (!start) return 0;
    return Math.round(((end || Date.now()) - start) / 1000);
  });
  ...
  const BOARD_PX = computed(() =>
    Math.max(200, Math.floor(CELL_PX * GRID_SIZE.value))
  );
