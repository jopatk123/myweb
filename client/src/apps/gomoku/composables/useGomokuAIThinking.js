...
export function useGomokuAIThinking(deps) {
  ...
  // 规则违反处理
  const { validateAIMove, getViolationAlert, clearViolation } =
    useAIRuleViolation();
    ...
    try {
      ...
      const aiResult = await gameModeService.getAIMove(
        ...
        thinkingUpdate => {
          ...
          if (debug)
            console.log('[Gomoku][AI] thinking update', thinkingUpdate);
        }
      );
      ...
      if (debug) {
        console.log('[Gomoku][AI] request snapshot', {
          ...
          moveHistorySnapshot: (gameHistory ? gameHistory.value : []).slice(
            -10
          ),
        });
      }
      ...
      // 验证AI移动是否违反规则
      const validationResult = validateAIMove(
        aiResult,
        board.value,
        playerNumber,
        playerNames
