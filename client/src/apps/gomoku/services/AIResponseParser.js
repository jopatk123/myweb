...
export function parseAIResponse(response) {
  try {
    ...
    if (fenced && fenced[1]) return validateGomokuResponse(JSON.parse(fenced[1].trim()));
    ...
    if (loose) {
      const row = parseInt(loose[1],10); const col = parseInt(loose[2],10);
      return validateGomokuResponse({ row, col, reasoning: response.slice(0,200) });
    }
    throw new Error('未找到有效JSON或坐标');
  } catch (e) { throw new Error(`AI回复格式错误: ${e.message}`); }
}
