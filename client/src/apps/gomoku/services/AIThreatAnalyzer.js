        ...
        for (const [dr, dc] of DIRECTIONS) {
          ...
          if (threat) {
            ...
            try {
              ...
              console.warn('analyzeThreats visited mark failed', e);
            }
          }
        }
      }
...
export function analyzeDirection(
  ...
  const continuousThreat = analyzeContinuousThreat(
    board,
    startRow,
    startCol,
    dr,
...
export function analyzeContinuousThreat(
  board,
  startRow,
  startCol,
  dr,
  ...
  let r = startRow - dr,
    c = startCol - dc;
  while (isValidCoordinate(r, c) && board[r][c] === playerType) {
    count++;
    leftEnd = { row: r, col: c };
    r -= dr;
    c -= dc;
  ...
  if (count === 2 && !blockedLeft && !blockedRight)
    return {
      type: '活二',
      count,
      leftEnd,
...
export function analyzeJumpThreat(
  board,
  startRow,
  startCol,
  dr,
...
export function findJumpThreatPattern(pattern, positions, playerType) {
  ...
  for (const p of jumpFourPatterns) {
    ...
    if (m) {
      return {
        type: '跳四',
        count: 4,
        leftEnd: positions[m.startIndex],
        rightEnd: positions[m.endIndex],
        gaps: m.gaps.map(i => positions[i]),
        continuous: false,
        pattern: p,
      };
    }
  ...
  for (const p of jumpThreePatterns) {
    ...
    if (m) {
      ...
      const rightBlocked =
        m.endIndex === pattern.length - 1 || pattern[m.endIndex + 1] !== 0;
      let jumpType;
...
export function findPatternInSequence(sequence, pattern, centerIndex) {
  ...
  for (
    let start = Math.max(0, centerIndex - patternLen);
    start <= Math.min(sequenceLen - patternLen, centerIndex);
    start++
  ) {
    let match = true;
    const gaps = [];
    ...
    if (match)
      return { startIndex: start, endIndex: start + patternLen - 1, gaps };
  }
  return null;
}
...
export function mergeThreats(threats) {
  ...
  return merged.sort(
    (a, b) => getThreatPriority(b.type) - getThreatPriority(a.type)
  );
}

export function getThreatPriority(type) {
...
export function formatThreatDescription(threat) {
  ...
  if (!continuous && gaps && gaps.length)
    desc += `，关键空位：${gaps.map(g => `(${g.row},${g.col})`).join(',')}`;
  return desc;
}
...
export function getBlockPositions(threat) {
  ...
  if (!threat.continuous && threat.gaps) {
    threat.gaps.forEach(g => {
      if (isValidCoordinate(g.row, g.col)) positions.push(g);
    });
    return positions;
  }
  ...
  if (!blockedLeft) {
    const leftBlock = { row: leftEnd.row - dr, col: leftEnd.col - dc };
    if (isValidCoordinate(leftBlock.row, leftBlock.col))
      positions.push(leftBlock);
  }
  if (!blockedRight) {
    const rightBlock = { row: rightEnd.row + dr, col: rightEnd.col + dc };
    if (isValidCoordinate(rightBlock.row, rightBlock.col))
