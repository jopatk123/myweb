  ...
  <div class="gomoku-room-lobby">
    ...
    <div class="form-row buttons">
      <button
        class="btn primary"
        @click="createRoom"
        :disabled="mpLoading || !mpForm.playerName.trim()"
      >
        创建房间
      </button>
      <button
        class="btn secondary"
        @click="joinRoom"
        :disabled="mpLoading || !mpForm.playerName.trim() || !mpForm.joinCode"
      >
        加入房间
      </button>
      <button
...
<script setup>
  ...
  async function joinRoom() {
    ...
    emit('update:mpLoading', true);
    try {
      const room = await mp.joinRoom(
        props.mpForm.playerName,
        props.mpForm.joinCode
      );
