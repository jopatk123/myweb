<template>
  <div class="mp-board-wrapper">
    ...
    <!-- 游戏结束弹窗 -->
    <div
      v-if="showGameEndDialog"
      class="game-end-dialog-backdrop"
...
<script setup>
  ...
  function applyGameState(gs) {
    ...
    mpBoard.winner =
      gs.winner !== undefined && gs.winner !== null ? Number(gs.winner) : null;
    // 仅当 winner 从无变为有时才弹窗
  ...
  watch(
    ...
    ev => {
      ...
      if (typeof ev.onGameUpdate === 'function')
        ev.onGameUpdate(handleGameUpdate);
  ...
  watch(
    () => unref(mp.gameStatus),
    s => {
      if (s === 'finished' && mpBoard.winner !== null)
        showGameEndDialog.value = true;
    },
    { immediate: true }
  );
