...
| `DEPLOY_VITE_API_BASE`      | 空                     | 部署期追加的 API 来源（与 `VITE_API_BASE` 一并加入 CSP connect-src，不互相覆盖）                                  |
...
| `LOG_DIR`                   | `server/logs`          | 日志目录；不配置时默认 `<repo>/server/logs`                                                                       |
| `LOG_FILE`                  | `<LOG_DIR>/server.log` | 日志文件完整路径；配置后 `LOG_DIR` 不再参与定位                                                                   |
