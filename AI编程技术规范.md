...
> 目标：为任何全新网站/应用项目在引入多款 AI 编程代理时提供统一的技术基线、协作流程与代码风格约束。这份规范以当前仓库的成熟实践为基础，但适用于不限领域的 Web 项目，可根据业务灵活裁剪。
...
- `README.md`：项目概览、快速开始、脚本说明、部署指南。
- `docs/architecture.md`：系统架构图、服务边界、数据流。
- `docs/api/`：API 说明或 OpenAPI 规范文件。
- `docs/environments.md`：环境变量、第三方服务、证书。
- `docs/playbook/`：运维手册、告警与应急预案。
